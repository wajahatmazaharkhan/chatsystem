export function calculateLoginFrequency(activities) {
  ...
  const uniqueLoginDays = new Set(
    loginActivities.map((activity) => {
      return new Date(activity.timestamp).toISOString().split("T")[0];
    }),
  );

  const loginDays = uniqueLoginDays.size;
...
export function generateChartData(activities) {
  const activityMap = {};
  ...
  const chartData = Object.entries(activityMap).map(([day, activity]) => {
    return {
      day,
      activity,
    };
...
export function formatActivityFeed(activities) {
  return (
    activities
